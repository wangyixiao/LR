...
class fNIRS_3D(nn.Module):  # n_class=3, sampling_point=sampling_points, dim=128, depth=6, heads=8, mlp_dim=64
    ...
    Args:
        ...
        sampling_point: sampling points of input fNIRS signals. Input shape is [B, 2, fNIRS channels, sampling points].采样点256
        dim: last dimension of output tensor after linear transformation.输出张量线性映射后的最后一维度/输入和输出维度
        depth: number of Transformer blocks.
        heads: number of the multi-head self-attention.多头自注意力的头的数目
        mlp_dim: dimension of the MLP layer.
        pool: MLP layer classification mode, 'cls' is [CLS] token pooling, 'mean' is  average pooling, default='cls'.使用cls token还是使用均值池化
        dim_head: dimension of the multi-head self-attention, default=64.多头自注意维度
