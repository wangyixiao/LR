...
import numpy as np
from STCT_3D import STCT_3D
from loadersi import Dataset, Load_Dataset_C, Split_Dataset_C
import os
...
if __name__ == "__main__":
    ...
    feature, label = Load_Dataset_C(data_path, 0, 25*25)

    _, _, sampling_points, channels = feature.shape  # (2250, 2, 256, 20)

    Subjects = 22
    for sub in range(1, Subjects+1):
        X_train, y_train, X_test, y_test = Split_Dataset_C(sub, feature, label, 25*25)
        ...
        net = STCT_3D(n_class=4, dim=160, depth=1, heads=4, mlp_dim=64, channel=24, point=25*25).to(device)

        criterion = OnlineLabelSmoothing(alpha=0.5, n_classes=4, smoothing=0.1)
        optimizer = torch.optim.AdamW(net.parameters(), lr=0.5)
